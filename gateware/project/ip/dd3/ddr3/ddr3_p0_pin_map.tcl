		...
		if {[array size results_array] == 1} {
			...
			if {[array size results_array] == 1} {
				...
				if {[array size pll_clk_results_array] == 1} {
					...
					ddr3_p0_traverse_fanin_up_to_depth $pll_inclk_id ddr3_p0_is_node_type_pin clock results_array 9
					if {[array size results_array] == 1} {
						set pin_id [lindex [array names results_array] 0]
						set result $pin_id
					} else {
...
proc ddr3_p0_get_ddr_pins { instname allpins } {
	...
	# AFI half clock
	set pll_afi_half_clock_id [ddr3_p0_get_output_clock_id $pins(afi_half_ck_pins) "AFI HALF CK" msg_list]
	if {$pll_afi_half_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "ddr3_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "ddr3_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_half_ck_pins)]"
	} else {
		set pll_afi_half_clock [ddr3_p0_get_pll_clock_name $pll_afi_half_clock_id]
	}
...
proc ddr3_p0_dump_static_pin_map { ddr_db_par filename } {
	...
	foreach instname $instnames {
		...
		ddr3_p0_static_map_expand_string $FH pins pll_afi_half_clock
