...
entity wfm_wcmd_fsm is
	...
	port (
		...
		wfm_load_ext			: out std_logic; -- to be removed
...
fsm : process(current_state, wfm_load, wfm_infifo_rdusedw, wrburst_cnt, wcmd_rdy) begin
	...
	case current_state is
		...
		when idle => --idle state wait for load signal
			if wfm_load='1' then
		...
		when check_burst => 	--wait for incoming data
		...
		when check_wr=>      --check if non burst write command can be issued
